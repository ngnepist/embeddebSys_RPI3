...
/*!
 *  @brief Function that swich on a LED during a time
 *
 * @param[in] period              : Delay in microseconds.
 * @return void.
 *
 */
void swich_on_LED(uint32_t period,int gpionumber);

/*!
 *  @brief Function that swich off a LED during a time
 *
 * @param[in] period              : Delay in microseconds.
 * @return void.
 *
 */
void swich_off_LED(uint32_t period, int gpionumber);

/*!
 *  @brief Function that handle crtl+c (SIGINT)
 *
 * @param[in] signum Number of the signal   : Delay in microseconds.
 * @return void.
 *
 */
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2)
    {
        ...
        while(1){ //la LED clignote car erreur
        swich_on_LED(1,24);
        swich_off_LED(1,24);
        }
        exit(1);
...
/*!
 *  @brief Function that swhich on a LED.
 */
void swich_on_LED(uint32_t period, int gpionumber){
    FILE* GPIO = NULL;
    if (gpionumber == 17){
    ...
    if (gpionumber == 24){
       GPIO = fopen("/sys/class/gpio/gpio24/value", "w");
    }
    if (GPIO != NULL)
    {
     fputc('1', GPIO);
     ...
     fclose(GPIO);
        ...
        fprintf(stderr, "error to opening GPIO%d\n", gpionumber);
    }
}

/*!
 *  @brief Function that swhich off a LED.
 */
void swich_off_LED(uint32_t period, int gpionumber){
    FILE* GPIO = NULL;
    if (gpionumber == 17){
